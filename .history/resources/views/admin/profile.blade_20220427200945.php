        ...
        <div class="panel-body ml-3">
            @if(!empty($latest_posts[0]))
                @foreach($latest_posts as $latest_post)
                            ...
                            <td><a href="{{ url('/'.$latest_post->slug) }}" class="mr-2 font-weight-bold">{{ $latest_post->title }}</a></td>
                            <td><span class="well-sm font-italic">{{ __('Published') }}: {{ $latest_post->created_at->format('Y-m-d, H:i') }}</span></td>
                        </tr>
                    </table>
                @endforeach
            @else
