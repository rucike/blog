    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-2">
                ...
                <script>
                    function getIndex() {
                    document.getElementById("demo").innerHTML = document.getElementById("mtype").selectedIndex;
                    }
                </script>
                    <form class="mt-2 mb-3 bg-white sm:rounded-lg p-2">
                        
                        <label for="type">{{ __('Choose the Type of Your Blogs') }}</label>
                        
                        <select id="type" name="type" class="block mt-1 w-full" onclick=>
                            <option name="blogType" value="11">{{ __('Show All') }}</option>
                            @foreach( $types as $type )
                                <option name="blogType" value="{{ $type->id }}">{{ $type->type }}</option>  
                            @endforeach
                        </select>
                        <input type="button" name="" onclick="{{ route('home','demo') }}" value="Filtruoti">
                    </form>
