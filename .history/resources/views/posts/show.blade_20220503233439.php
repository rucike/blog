                            ...
                            <div class="list-group-item">
                                <p>{{ $comment->body }}
                                    @if(!Auth::guest() && ($comment->from_user == Auth::user()->id || Auth::user()->is_admin()))
                                        <!--<button class="btn" style="float:right; background-color:#d279a6;"><a href="{{ url('comment/delete/'.$comment->id)}}">{{ __('Delete') }}</a></button>-->
                                        <a style="float:right; background-color:#d279a6;" href="{{  url('comment/delete/'.$comment->id.'?_token='.csrf_token()) }}" class="btn btn-danger">{{ __('Delete') }}</a>
                                    @endif
                                </p>
                            </div>
                        </div>
