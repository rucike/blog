...
Route::middleware(['auth'])->group(function () {
  ...
  //users profile
  Route::get('user/{id}', [UserController::class, 'profile'])->where('id', '[0-9]+')->name('user/'.Auth::id())->middleware('verified');;
  // display list of posts
  Route::get('user/{id}/posts', [UserController::class, 'user_posts'])->where('id', '[0-9]+')->middleware('verified');;
  // display single post
  //Route::get('/{slug}', ['as' => 'post', 'uses' => 'PostsController@show'])->where('slug', '[A-Za-z0-9-_]+');
 Route::get('/{slug}', [PostsController::class, 'show'])->where('slug', '[A-Za-z0-9-_]+')->middleware('verified');;